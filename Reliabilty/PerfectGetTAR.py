...
tree = ptml_importer.apply("D:/Desktop/临时/running-example.ptml")
...
# def originalPosition(xnode,new_child):
#     node_parent = xnode._get_parent()
#     for i in range(len(node_parent.children)):
#         print("node_parent.children[i]++++++++++++++++",node_parent.children[i])
#     leaf_position_dic = dict()
#     leaf = node_parent.children[i]
#     position = i
...
def get_PT_TARSet(ProcessTree0):
    ...
    #             print("t_leaves._get_parent().children+++++++++++++++++--------------------+++++++++++++++++",t_leaves[i]._get_parent().children[0])
    ...
    while(deep >= 2):
        ...
        # 遍历父节点列表，treelist2[abc]为第1个父节点列表
        for abc in range(len(treelist2)):
            print("abc", abc, "+treelist2:", treelist2[abc])
            # 获取父节点列表的所有叶子结点，并给他们存入字典中，key是叶子结点，value是他们在父节点中的索引
            dic0 = get_operator_child_dic(treelist2[abc])
            print("dic00000000000000000000000000000000", dic0)
            leaf_position_dic = dict()
            # key是叶子结点，value是他们在父节点中的索引,dic0.values()是叶子结点列表
            print("dic0.values(),,,,,,,,,,,,", list(dic0.values())[0])
            for item in range(len(list(dic0.values())[0])):
                print("item", item, "list[item]", list(dic0.values())[0][item])
                leaf = list(dic0.values())[0][item]
                position = item
                leaf_position_dic.update({leaf: position})
                print("leaf_position_dic111111", leaf_position_dic)
            print("leaf_position_dic222222", leaf_position_dic)
            # 遍历所有叶子结点，如果是单独叶子结点，就调用leaf2node，如果是已经重命名过得，就调用operator2Node
            ...
            for item in range(len(list(dic0.values())[0])):
                ...
                if (contain_rename_node(list(dic0.values())[0][item]) == False):
                    ...
                    # gviz = pt_visualizer.apply(ProcessTree0,
                    #                            parameters={
        ...
        for operatorTree in treelist2:
            if(operatorTree._get_parent() != None):
                print("operatorTree////////////////", operatorTree._get_parent())
                operator_position_dic = dict()
                # key是operator结点，value是他们在父节点中的索引
                ...
                for yy in range(len(operatorTree._get_parent().children)):
                    print("yy", yy, "operatorTree._get_parent().children[yy]", operatorTree._get_parent().children[yy])
                    opera = operatorTree._get_parent().children[yy]
                    opPosition = yy
                    operator_position_dic.update({opera: opPosition})
                    print("operator_position_dic6666666", operator_position_dic)
                print("operator_position_dic222677777777222", operator_position_dic)
                newOpNode = Operator2Label(operatorTree)
                print("newOpNode", newOpNode)
                index = operator_position_dic.get(operatorTree)
                print("index----------------->", index)
                print("operatorTree._get_parent().children[index]", operatorTree._get_parent().children[index])
    ...
    #
    # # 得到最底层叶子结点
    # t_leaves2 = t(ProcessTree0)
    # print("t_leaves2t_leaves2t_leaves2t_leaves2t_leaves2t_leaves2t_leaves2::::",t_leaves2)
    # # 得到最底层叶子结点的父节点列表
    # treelist3 = use_parent_classify_leaves(t_leaves2)
    # # 遍历父节点列表，treelist2[abc]为第1个父节点列表
    # for aaa in range(len(treelist3)):
    #     print("aaa", aaa, "+treelist3:", treelist3[aaa])
    #     # 获取父节点列表的所有叶子结点，并给他们存入字典中，key是叶子结点，value是他们在父节点中的索引
    #     dic1 = get_operator_child_dic(treelist3[aaa])
    #     print("dic111111111111111111111111111111111", dic1)
    #     # key是叶子结点，value是他们在父节点中的索引,dic0.values()是叶子结点列表
    #     print("dic1111111.values(),,,,,,,,,,,,", list(dic1.values())[0])
    #     # 遍历所有叶子结点，如果是单独叶子结点，就调用leaf2node，如果是已经重命名过得，就调用operator2Node
    #     nodesParent = list(dic1.values())[0][0]._get_parent()
    #     for item in range(len(list(dic1.values())[0])):
    #         print("item", item, "list[item]", list(dic1.values())[0][item])
    #         if (contain_rename_node(list(dic1.values())[0][item]) == False):
    #             newNode = LeafNode2Label(list(dic1.values())[0][item])
    #             print("newNode", newNode)
    #             nodesParent.children[item] = newNode
    #
    #             gviz = pt_visualizer.apply(ProcessTree0,
    ...
    #             continue;
    #
    #     print("list(dic0.values())[0][0]._get_parent()***************", list(dic1.values())[0][0]._get_parent())
    #     for i in range(len(list(dic1.values())[0])):
    #         print("nodesParent.children[i]", nodesParent.children[i])
    #
    # for operatorTree in treelist3:
    #     Operator2Label(operatorTree)
    # gviz = pt_visualizer.apply(ProcessTree0,
    #                            parameters={
    #                                pt_visualizer.Variants.WO_DECORATION.value.Parameters.FORMAT: "png"})
    ...
    #         #遍历所有叶子结点，如果是单独叶子结点，就调用leaf2node，如果是已经重命名过得，就调用operator2Node
    #         for item in range(len(list(dic0.values())[0])):
    #             if (contain_rename_node(list(dic0.values())[0][item]) == True):
    #                 print("~~~~~~~~~~~~~~~~~~~~True~~~~~~~~~~~~~~~")
    #                 print("~~~~~~~~~~~list(dic0.values())[0][item]",list(dic0.values())[0][item])
    #                 # print("~~~~~~~~~~~list(dic0.values())[0][item][0]", list(dic0.values())[0][item])
    #                 Operator2Label(list(dic0.values())[0][item])
    #                 gviz = pt_visualizer.apply(ProcessTree0,
    #                                            parameters={
    #                                                pt_visualizer.Variants.WO_DECORATION.value.Parameters.FORMAT: "png"})
    #                 pt_visualizer.view(gviz)
    #                 continue;
    #             if (contain_rename_node(list(dic0.values())[0][item]) == False):
    #                 LeafNode2Label(list(dic0.values())[0][item])
    #                 gviz = pt_visualizer.apply(ProcessTree0,
    ...
    #     print("deep is ====> ", deep)
        ...
        #     if (contain_rename_node(t_leaves[i]) == False):
        #         LeafNode2Label(t_leaves[i])
        #         print("t_leaves._get_parent().children+++++++++++++++++--------------------+++++++++++++++++",
        #               t_leaves[i]._get_parent().children[0])
        # t_leaves0 = t(ProcessTree0)
        # for z in range(len(t_leaves0)):
        #     if (contain_rename_node(t_leaves0[z]) == False):
        #         LeafNode2Label(t_leaves0[z])
