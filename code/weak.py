...
# 读取数据
# ==========================
df = pd.read_excel("weak_data.xlsx", header=None)
...
# ==========================
results = []
alg_IRI_dict = {alg: [] for alg in alg_names}

for i in range(len(df)):

    row = df.iloc[i, 0:10]  # 前10列是数据
...
# 保存
# ==========================
df.to_excel("weak_results.xlsx", index=False, header=False)
